Shader "HDRP/Lit_TimeInteractable2"
{
    Properties
    {
        ...
        [HDR]_HighlightColour("Highlight Colour", Color) = (1.414675,1.421379,1.416956,1)
...
0, 0, 1.05
            ...
            float4 PopulateSurfaceData()
            {
                float4 surface;
                float _Remap_Out_3;
                Unity_Remap_float(sin(_Time.y * 3), float2 (-1, 1), float2 (0, 1), _Remap_Out_3);
                float _Branch_Out_3;
                Unity_Branch_float(_IsTwinkling, _Remap_Out_3, 1, _Branch_Out_3);
                float4 _Multiply_Out_2 = _HighlightColour * _Branch_Out_3.xxxx;
                Unity_Branch_float4(_IsHighlighted, _Multiply_Out_2, float4(0, 0, 0, 0), surface);
                return surface;
            }
            ...
            GraphVertexOutput vert(GraphVertexInput v)
            {
                ...
                GraphVertexOutput o;
                float3 pos = mul(_ScaleMatrix, v.vertex.xyz);
                o.position = TransformObjectToHClip(pos);

                o.color = PopulateSurfaceData();
                return o;
            }

            float4 frag(GraphVertexOutput IN) : COLOR
            {                
                return all(isfinite(IN.color)) ? IN.color : float4(1.0f, 0.0f, 1.0f, 1.0f);
            }
